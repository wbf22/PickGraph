...
import org.pickgraph.annotations.testObjects.LargeChildPGO;
import org.pickgraph.annotations.testObjects.LargePGO;
import org.pickgraph.annotations.testObjects.TestPGO1;
import org.pickgraph.annotations.testObjects.fromDemo.Vendor;
import org.pickgraph.config.PickGraphAutoConfig;
import org.pickgraph.config.PickGraphProperties;
import org.pickgraph.util.Timer;
...
import static org.pickgraph.util.Timer.start;
...
class PickGraphTest {
    ...
    void test_execute_largeObject() {
        ...
        Timer lt = start();
        Map<String, Object> lresult = pickGraph.execute(lfields, LargePGO.class,null, SNAKE_CASE);
        lresult = pickGraph.execute(lfields, LargePGO.class,null, SNAKE_CASE);
        lresult = pickGraph.execute(lfields, LargePGO.class,null, SNAKE_CASE);
...
//        }
        ...
        Map<String, Object> fields = new HashMap<>();
        fields.put("vendorIdentifier", null);
        fields.put("name", null);
        fields.put("description", null);

        Map<String, Object> addressFields = new HashMap<>();
        addressFields.put("city", null);
        addressFields.put("state", null);
        addressFields.put("zipCode", null);
        fields.put("address", addressFields);

        fields.put("email", null);

        Map<String, Object> result = pickGraph.execute(fields, Vendor.class,null, SNAKE_CASE);

        assertNotNull(result);
        assertEquals("ven_2kk2k3j4dikdf2", result.get("vendor_identifier"));
        assertEquals("Acme Corp", result.get("name"));
        assertEquals("Acme Corp is a company that makes things.", result.get("description"));
        assertFalse(result.containsKey("phoneNumber"));
        assertEquals("ceryl@acmecorp.com", result.get("email"));
        ...
        assertEquals("12345", address.get("zip_code"));
